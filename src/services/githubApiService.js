...
class GitHubApiService {
    ...
    `;
        try {
            const response = await this.graphqlClient(query, { projectId });
            const project = response.node;
            // Procesar campos personalizados
            const customFields = project.fields.nodes.map((field) => ({
                id: field.id,
                name: field.name,
            ...
            const tasks = project.items.nodes
                .filter((item) => item.content && (item.content.__typename === 'Issue' || item.content.__typename === 'DraftIssue'))
                .map((item) => {
    ...
    `;
        try {
            ...
            const response = await this.graphqlClient(createIssueMutation, {
                repositoryId: repoId,
                title: params.title,
                body: params.body || '',
    ...
    `;
        const response = await this.graphqlClient(query, { owner, name });
